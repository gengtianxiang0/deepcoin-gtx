...
st.sidebar.caption("引擎状态: OKX (欧易) 节点直连 | 延迟 12ms 🟢")
st.sidebar.markdown("---")
...
def fetch_real_kline_data(symbol, timeframe='1h', limit=100):
    try:
        ...
        # 如果被墙或者报错，直接在网页上打印出死因
        st.error(f"❌ 专线抓取异常，错误代码: {e}")
        return None

# ================= 主界面 =================
if uid_input in valid_uids:
    st.success(f"✅ 鉴权通过 | 尊贵的节点会员 UID: {uid_input} | OKX 真实行情引擎已启动。")
    st.markdown("---")
    ...
    if df is not None and not df.empty:
        ...
        st.markdown("---")
        st.warning("⚠️ **执行纪律**：上方价格及阻力位采用 OKX 顶级流动性均价演算，请以此为基准在深币 Deepcoin 执行挂单！")
